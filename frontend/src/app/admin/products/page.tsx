...
import { Product, getProducts, uploadProduct, deleteProduct, API_URL } from '@/lib/api';
...
export default function AdminProducts() {
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    try {
      await uploadProduct(formData, adminKey);
      fetchProducts();
      setFile(null);
      setName('');
    ...
    <div className="container mx-auto p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {products.map((product) => (
          <div key={product.id} className="bg-white rounded shadow p-4 dark:bg-gray-800 relative group">
            <img src={getImageUrl(product.image_url)} alt={product.name} className="w-full h-48 object-cover rounded mb-2" />
